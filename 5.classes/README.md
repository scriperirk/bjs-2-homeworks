...
5. Создайте класс `Magazine`, который будет наследоваться от класса `PrintEditionItem`. Конструктор класса должен принимать такие же параметры, как и класс-родитель. От базового печатного издания журнал отличается только типом. Значение свойства `type` должно быть равно `"magazine"`.
...
Теперь с помощью классов можно описать все печатные издания, которые бывают в библиотеке. Следующая задача — подготовить библиотеку к работе в цифровом режиме, то есть реализовать возможности хранить книги, выдавать их читателям и принимать обратно.
...
3. Создайте метод `findBookBy(type, value)`, который в качестве аргумента будет принимать ключ, по которому идёт поиск (тип, автор, название, год выпуска и пр.) и искомое значение. Метод должен возвращать книгу в случае успеха и `null`, если запрошенная книга не найдена.
...
Мы продолжаем работать с журналом успеваемости студентов. Нам нужно **преобразовать код прошлого ДЗ к классам**, а также добавить возможности:

- внести оценки по названию предмета,
- получить среднюю оценку по предмету,
- получить среднюю оценку по всем предметам,
- вывести сообщение об ошибке при неверной оценке (не в промежутке 1-5). Оценка в этом случае не заносится в журнал.

Вам предстоит придумать структуру данных для хранения оценок по предметам. Работать это должно следующим образом:
